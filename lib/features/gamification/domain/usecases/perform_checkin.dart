...
class PerformCheckin {
  ...
  Future<PerformCheckinResult> call(PerformCheckinParams params) async {
    // ═══════════════════════════════════════════════════
    // ⚠️ ВРЕМЕННЫЙ БАЙПАС ГЕОЛОКАЦИИ ДЛЯ ТЕСТИРОВАНИЯ UI
    // Раскомментируй этот блок перед релизом в продакшен!
    // ═══════════════════════════════════════════════════
    /*
    if (params.accuracy > 100) {
      return const PerformCheckinFailure(
    ...
    if (distance > 300) {
      return PerformCheckinFailure(
        'Вы слишком далеко от локации. Расстояние: ${distance.toInt()} м.',
      );
    }
    */
    // ═══════════════════════════════════════════════════

    // 1. Проверка, не посещали ли уже сегодня (оставляем, чтобы тестировать защиту от дублей)
    final history = await journalRepo.getAllVisits();
    final now = DateTime.now();
    final today = DateTime(now.year, now.month, now.day);
  ...
  // Эта функция теперь не вызывается, но оставлена на случай, если ты вернёшь проверку
  double _calculateDistance(
    double lat1,
    double lon1,
    double lat2,
